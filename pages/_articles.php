...
// =================== STARTARTIKEL-CARD ===================
?>
<div class="card rex-sr-card mb-3 rex-sr-startcard">
    <div class="card-body">
        <?php if ($startArticle):
        ...
        ?>
            <div class="d-flex align-items-center gap-3 rex-sr-startarticle-row" data-art-id="<?= $startArticle->getId() ?>" data-clang="<?= $clang ?>">
                <i class="rex-icon <?= $saIcon ?> fa-2x text-warning"></i>
                <div class="flex-grow-1">
                    <div class="text-muted small d-flex align-items-center gap-2">
                        <span><?= rex_i18n::msg('structure_replace_startarticle') ?></span>
                        <span class="badge text-bg-light text-muted">#<?= $startArticle->getId() ?></span>
                        <span class="<?= rex_escape($saType[1]) ?>"><i class="rex-icon <?= rex_escape($saType[2]) ?>"></i> <?= rex_escape($saType[0]) ?></span>
                    </div>
                    <?php if ($perms['editArt'] && $perms['hasCatPerm']): ?>
                        <input type="text" class="form-control form-control-lg rex-sr-inline" data-sr-inline="art" data-sr-field="name" data-sr-id="<?= $startArticle->getId() ?>" value="<?= rex_escape($startArticle->getName()) ?>" maxlength="255" />
                    <?php else: ?>
                ...
                <div class="d-flex gap-2 align-items-center">
                    <?php if ($contentAvailable && $perms['editArt'] && $perms['hasCatPerm']): ?>
                        <select class="form-select form-select-sm rex-sr-inline rex-sr-template-select" data-sr-inline="art" data-sr-field="template_id" data-sr-id="<?= $startArticle->getId() ?>">
                            <?php foreach ($templateNames as $tid => $tname): ?>
                                <option value="<?= (int) $tid ?>" <?= $startArticle->getTemplateId() === (int) $tid ? 'selected' : '' ?>><?= rex_escape($tname) ?></option>
                            <?php endforeach; ?>
                        </select>
                    <?php endif; ?>
                    <a class="btn btn-sm btn-primary" href="<?= rex_escape($saEditUrl) ?>"><i class="rex-icon rex-icon-edit-mode"></i> <?= rex_i18n::msg('structure_replace_edit_content') ?></a>
                </div>
            </div>
        <?php elseif ($categoryId > 0): ?>
            <em class="text-muted"><?= rex_i18n::msg('structure_replace_no_articles') ?></em>
        <?php endif; ?>
    </div>
</div>
        ...
        <table class="table table-hover align-middle mb-0 rex-sr-art-table" data-dnd-active="<?= $dndActive ? '1' : '0' ?>">
            <thead>
                <tr>
                    ...
                    <th class="rex-sr-col-name" scope="col"><?= $sortHeader('name', rex_i18n::msg('header_article_name')) ?></th>
                    <?php if ($contentAvailable): ?>
                        <th class="rex-sr-col-template" scope="col"><?= rex_i18n::msg('header_template') ?></th>
                    <?php endif; ?>
                    <th class="rex-sr-col-status" scope="col"><?= $sortHeader('status', rex_i18n::msg('header_status')) ?></th>
                    <th class="rex-sr-col-prio" scope="col"><?= $sortHeader('priority', rex_i18n::msg('header_priority')) ?></th>
                    <th class="rex-sr-col-updated" scope="col"><?= $sortHeader('updatedate', rex_i18n::msg('updated_on')) ?></th>
                    <th class="rex-sr-col-actions" scope="col" aria-label=""></th>
                </tr>
            </thead>
            <tbody data-sortable="articles" data-category-id="<?= $categoryId ?>">
            <?php
            if (count($articles) === 0): ?>
                <tr><td colspan="<?= $contentAvailable ? 8 : 7 ?>" class="text-muted text-center py-3"><?= rex_i18n::msg('structure_replace_no_articles') ?></td></tr>
            <?php else:
                foreach ($articles as $art):
                    ...
                    ?>
                    <tr class="rex-sr-art-row" data-art-id="<?= $id ?>" data-priority="<?= (int) $art->getPriority() ?>" data-clang="<?= $clang ?>">
                        ...
                        <td class="rex-sr-col-name">
                            ...
                            <?php else: ?>
                                <a class="rex-link-expanded" href="<?= rex_escape($editUrl) ?>"><?= rex_escape($art->getName()) ?></a>
                            <?php endif; ?>
                        </td>
                        <?php if ($contentAvailable): ?>
                            <td class="rex-sr-col-template">
                                <?php if ($canEdit): ?>
                                    <select class="form-select form-select-sm rex-sr-inline rex-sr-template-select" data-sr-inline="art" data-sr-field="template_id" data-sr-id="<?= $id ?>">
                                        <?php foreach ($templateNames as $tid => $tname): ?>
                                            <option value="<?= (int) $tid ?>" <?= $art->getTemplateId() === (int) $tid ? 'selected' : '' ?>><?= rex_escape($tname) ?></option>
                                        <?php endforeach; ?>
                                    </select>
                                <?php else: ?>
                                    <span class="text-muted small"><?= rex_escape($templateNames[(int) $art->getValue('template_id')] ?? '') ?></span>
                                <?php endif; ?>
                            </td>
                        <?php endif; ?>
